...
const openPosition = async (
  ...
  try {
    ...
    // weight amount based on confidence from tensorflow
    ...
    newPosition.amount = Number.parseFloat(amount).toPrecision(2)

    if (MODE === "live") {
      let order = await exchanges.placeOrder(
        newPosition.pair,
        newPosition.amount // TODO: allow EXCHANGE_LIMIT in placeOrder
      )
...
const getPrediction = async (predictionObject, pair) => {
  return new Promise(async (resolve, reject) => {
    ...
    exec(
      `gcloud ml-engine predict --model jimini --version v7 --json-instances ./tmp/predictionObject-${pair}.json`,
      (err, stdout, stderr) => {
        if (err) {
          // node couldn't execute the command
          reject(err)
